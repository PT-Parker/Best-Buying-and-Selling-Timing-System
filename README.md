...
## 功能亮點
- **即時決策卡**：最新行情＋模型機率＋期望報酬，低於門檻或期望報酬為負時自動顯示觀望；達標可一鍵加入待執行清單（需按鈕觸發）。
...
- **Gemini 解讀（手動）**：決策卡與價格預測可按「生成 Gemini 解讀」取得 2-3 行解釋。
- **多代理決策（手動）**：勾選後按「執行多代理推理」才呼叫 Gemini。
- **三角色專家解讀**：多頭／空頭／中立三位專家會給分，最高分專家以第一人稱說明理由。
...
- 側邊欄「Gemini API 設定」可輸入 `GEMINI_API_KEY`，按「啟動 Gemini」後本次 Session 生效。  
- 按「更新行情並生成今日決策卡」取得最新建議；若顯示「考慮進場」可加入待執行清單。  
- 若需更新模型，按「以目前資料重新訓練模型」；訓練後需再按「更新行情並生成今日決策卡」。  
- 「生成價格預測」可查看未來 N 日價格推估。  
- 多代理模式需勾選「使用多代理決策 (Beta)」並按「執行多代理推理」。  
- 決策卡與價格預測的「Gemini 解讀」為手動觸發，按「生成 Gemini 解讀」才會呼叫 API。
...
- Gemini 專家推理為強制模式，需設定 `GEMINI_API_KEY`（或側邊欄輸入／Streamlit secrets），否則多代理模式會停用。
- Gemini 呼叫皆改為手動按鈕觸發，以避免配額過快耗盡。
